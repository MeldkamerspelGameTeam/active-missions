...
def summarize_readme_markdown(missions: list[dict[str, Any]], never_seen: list[dict[str, Any]], old_seen: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f'    "Seen last 30 days" : {active_seen_last_30_days}',
        f'    "Never seen" : {never_seen_active}',
        f'    "Old seen 30+ days" : {old_seen_active}',
    ]

    if active_other_count > 0:
        lines.append(f'    "Other/unknown seen state" : {active_other_count}')
